...
const toggleTranslation = async (req: Request, res: Response, next: NextFunction) => {
  const id = parseInt(req.params.id, 10);
...
const toggleSelectedTranslations = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { ids } = req.body;
